...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\draw[thick, retractionColor, -latex] (Uk) to[out=\gradientAngleA, in=130] ++(0.18*\distance, -0.08*\distance) to[out=-50, in=180] ++(0.16*\distance, -0.11*\distance) to[out=0, in=130] ++(0.29*\distance, -0.03*\distance);
		...
		\node[] () at (\UkpPosX-0.05*\distance, \UkpPosY-0.1*\distance) {$U_{k+1}$};
