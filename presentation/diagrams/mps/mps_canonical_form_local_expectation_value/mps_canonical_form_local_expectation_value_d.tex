...
\begin{document}
	...
	\begin{tikzpicture}
		...
		% Open Legs
		\begin{pgfonlayer}{bg}
			\draw[virtualLeg](A4)+({(-\tensorDistanceMPS+\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- (A4);
			\draw[virtualLeg](A4cc)+({(-\tensorDistanceMPS+\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- (A4cc);
			\draw[virtualLegWithoutArrows] (A4) -- ++({(-\tensorDistanceMPS+\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- ++(0, -\braketDistanceMPS) -- (A4cc);
			\draw[virtualLeg](A5)+({(\tensorDistanceMPS-\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- (A5);
			\draw[virtualLeg](A5cc)+({(\tensorDistanceMPS-\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- (A5cc);
			\draw[virtualLegWithoutArrows] (A5) -- ++({(\tensorDistanceMPS-\tensorWidthMPS/2)*\openLegMultiplierMPS}, 0) -- ++(0, -\braketDistanceMPS) -- (A5cc);
		\end{pgfonlayer}
	\end{tikzpicture}
\end{document}